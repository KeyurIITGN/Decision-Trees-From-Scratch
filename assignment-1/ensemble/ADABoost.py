...
class AdaBoostClassifier():
    ...
    def plot(self):
        ...
        fig1=plt.figure(1,figsize=[3*self.n_estimators,2*self.n_estimators])
        for i in range(self.n_estimators):
            ...
            fig1.tight_layout(h_pad=0.5, w_pad=0.5, pad=2.5)
        ...
        plt.axis("tight")
        plt.show()
        #plt.savefig('ADABoost_estimators_surface'+ '.png')

		# tree structure
        fig2=plt.figure(2)
        mdl = classifier.fit(X,y)
        plot_tree(mdl, filled=True)
        #plt.savefig('ADABoost' + 'tree.png')
        ...
        plt.savefig('ADABoost_combined_surfaces'+'.png')
        plt.show()
        return fig1,fig3
