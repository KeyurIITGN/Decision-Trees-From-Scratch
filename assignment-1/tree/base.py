...
from .utils import entropy, gain_with_gini, information_gain, gini_index, variance
...
class DecisionTree():
    ...
    def fit(self, X, y):
        ...
        if type(num)==np.int64 or type(num)==np.int32 or type(num)==int:
            ...
            num = y.values[0]
            if type(num)==np.int64 or type(num)==np.int32 or type(num)==int:
                self.tree = self.real_input_discrete_output(self.tree, self.X, self.y, 0)
            else:
    ...
    def max_infogain_attribute(self, dataset, labels):
            ...
            if self.criterion=="information_gain":
                ...
                # gini index criteria
                for j in dataset:
                    temp_entropy = gain_with_gini(labels, dataset[j])
                    if temp_entropy>max_entropy:
                        max_entropy = temp_entropy
                        chosen_attribute = j
